...
ARCHITECTURE tb OF axi_pkg_tb IS
  ...
  SIGNAL s_axi_in  : t_axi_ports_in;
  SIGNAL s_axi_out : t_axi_ports_out;

  -- Smoke check that the array types elaborate correctly.
  SIGNAL s_axi_in_array  : t_axi_ports_in_array(0 TO 1);
  SIGNAL s_axi_out_array : t_axi_ports_out_array(0 TO 1);
...
BEGIN
  ...
  s_axi_out.S_AXI_AWREADY <= s_axi_in.S_AXI_AWVALID;
  s_axi_out.S_AXI_WREADY  <= s_axi_in.S_AXI_WVALID;
  s_axi_out.S_AXI_BVALID  <= s_axi_in.S_AXI_BREADY;
  s_axi_out.S_AXI_BRESP   <= "00"; -- OKAY
  ...
  BEGIN
    ...
    WHILE test_suite LOOP
      ...
      IF run("test_axi_write_drives_address_and_data") THEN
        axi_write(X"001234", X"DEADBEEF", s_clk, s_axi_in, s_axi_out);
        WAIT FOR 0 ns; -- let axi_write's final signal assignments settle

        check_equal(STD_LOGIC_VECTOR(s_axi_in.S_AXI_AWADDR), STD_LOGIC_VECTOR'(X"001234"), "AWADDR should hold the written address");
        check_equal(STD_LOGIC_VECTOR(s_axi_in.S_AXI_WDATA), STD_LOGIC_VECTOR'(X"DEADBEEF"), "WDATA should hold the written data");
        check_equal(s_axi_in.S_AXI_AWVALID, '0', "AWVALID should be deasserted after the handshake");
        check_equal(s_axi_in.S_AXI_WVALID, '0', "WVALID should be deasserted after the handshake");
        check_equal(s_axi_in.S_AXI_BREADY, '0', "BREADY should be deasserted after the write response");

      ELSIF run("test_axi_write_back_to_back") THEN
        axi_write(X"000010", X"11111111", s_clk, s_axi_in, s_axi_out);
        axi_write(X"000020", X"22222222", s_clk, s_axi_in, s_axi_out);
        WAIT FOR 0 ns; -- let axi_write's final signal assignments settle

        check_equal(STD_LOGIC_VECTOR(s_axi_in.S_AXI_AWADDR), STD_LOGIC_VECTOR'(X"000020"), "Second AWADDR should overwrite the first");
        check_equal(STD_LOGIC_VECTOR(s_axi_in.S_AXI_WDATA), STD_LOGIC_VECTOR'(X"22222222"), "Second WDATA should overwrite the first");

      ELSIF run("test_axis_push_handshake") THEN
        s_axis_tready <= '1';
        axis_push(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, X"A5");
      ...
      ELSIF run("test_axis_push_with_tlast") THEN
        s_axis_tready <= '1';
        axis_push(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, X"5A", s_axis_tlast, '1');
        check_equal(s_axis_tvalid, '0', "axis_push should deassert tvalid after handshake");
      ...
      ELSIF run("test_axis_pop") THEN
        s_axis_tvalid <= '1';
        s_axis_tdata  <= X"42";
        axis_pop(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, v_captured_data);
        ...
        s_axis_tdata <= X"BE";
        s_axis_tlast <= '1';
        axis_pop(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, v_captured_data, s_axis_tlast, v_captured_last);
      ...
      ELSIF run("test_stream_wrappers") THEN
        s_axis_tready <= '1';
        push_stream(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, X"3C");
        ...
        s_axis_tvalid <= '1';
        s_axis_tdata  <= X"7E";
        pop_stream(s_clk, s_axis_tvalid, s_axis_tready, s_axis_tdata, v_captured_data);
